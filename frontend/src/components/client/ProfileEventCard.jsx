...
import { Notebook, Users, UserStar } from 'lucide-react';
...
const ProfileEventCard = ({ reg, showActions = true, onCancelRegistration, onViewDetails, onViewTeam, onViewQRCode }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition-all duration-200 hover:border-blue-300">
      {/* Event Header */}
      <div className="px-4 py-6 lg:py-4 border-b border-gray-100">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-gray-900 text-base mb-1 lg:mb-2 group-hover:text-blue-700 transition-colors leading-tight">
                {reg.event.event_name}
              </h3>
              <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 lg:gap-6 text-sm text-gray-600">
          ...
          <div className="space-y-3">
            ...
            {reg.registration?.registration_type === 'individual' ? (
              <>
                ...
                {/* Desktop layout for individual registrations - Simple single line */}
                <div className="hidden lg:flex lg:items-center lg:justify-between lg:gap-3">
                  {/* Left side: Type and ID inline */}
                  <div className="flex items-center gap-3">
                    <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200">
                      <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                      </svg>
                      Individual
              ...
              /* For non-individual registrations - Compact desktop layout */
              <>
                {/* Registration Type Badge and ID - Mobile/Tablet */}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {reg.registration?.registration_type && (
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium ${(reg.registration.registration_type === 'team_leader' || reg.registration.registration_type === 'team')
                        ...
                        {reg.registration.is_team_leader || reg.registration.registration_type === 'team_leader' ? `Team Lead${teamName ? ` : ${teamName}` : ''}` :
                ...
                {/* Desktop layout for team registrations - Simple single line */}
                <div className="hidden lg:flex lg:items-center lg:justify-between lg:gap-3">
                  {/* Left side: Team info inline */}
                  <div className="flex items-center gap-3">
                    <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded text-xs font-medium ${(reg.registration.registration_type === 'team_leader' || reg.registration.registration_type === 'team')
                      ? 'bg-purple-50 text-purple-700 border border-purple-200'
                      : reg.registration.registration_type === 'team_member'
                        ? 'bg-orange-50 text-orange-700 border border-orange-200'
                        : 'bg-slate-50 text-slate-700 border border-slate-200'
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {(reg.registration?.registration_type === "team_leader" || reg.registration?.is_team_leader) && (
                      <Link
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-md transition-colors"
                      >
                    ...
                    {shouldShowViewTeamButton && (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-purple-700 bg-purple-50 hover:bg-purple-100 border border-purple-200 rounded-md transition-colors"
                      >
                        <Users className='w-3 h-3' />
                        View Team
                      </button>
                ...
                <div className="flex flex-wrap sm:flex-row gap-2 lg:hidden">
                  <button
                    onClick={() => onViewQRCode && onViewQRCode({
                      registration: reg.registration,
                      event: reg.event,
