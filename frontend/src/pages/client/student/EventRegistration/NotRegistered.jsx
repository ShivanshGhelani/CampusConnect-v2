...
const StudentNotRegistered = () => {
  ...
  const checkRegistrationStatus = async (eventData) => {
    try {
      ...
      if (eventData.target_audience !== 'students' && eventData.target_audience !== 'both') {
        setRegistrationError('This event is not available for student registration.');
        return;
      }
  ...
  const getRegistrationOptions = () => {
    ...
    if (event.registration_type === 'both' || event.registration_type === 'individual') {
      options.push({
        ...
        link: `/student/events/${eventId}/register/individual`,
        color: 'blue'
      });
    }
    ...
    if (event.registration_type === 'both' || event.registration_type === 'team') {
      options.push({
        ...
        description: `Register as a team (${event.team_size_min || 2}-${event.team_size_max || 5} members)`,
        link: `/student/events/${eventId}/register/team`,
        color: 'purple'
      });
    }
  ...
  const canRegister = () => {
    ...
    return (
      ...
      (event.target_audience === 'students' || event.target_audience === 'both') &&
...
export default StudentNotRegistered;
