...
/**
 * FacultyAlreadyRegistered - Page shown when faculty is already registered for an event
 */
const FacultyAlreadyRegistered = () => {
    ...
    const loadRegistrationDetails = async () => {
      ...
      try {
        ...
        if (registrationResponse.data.is_registered) {
          ...
          if (registrationResponse.data.registration.registration_type === 'team' ||
              registrationResponse.data.registration.registration_type === 'team_leader' ||
              registrationResponse.data.registration.registration_type === 'team_participant') {
            try {
  ...
  const handleManageTeam = () => {
    navigate(`/faculty/events/${eventId}/manage-team`);
  };

  const handleCompletePayment = () => {
    navigate(`/faculty/events/${eventId}/payment`);
  };
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-md border border-gray-200 mb-6">
            ...
            <div className="p-6">
                ...
                <div className="bg-purple-50 border border-purple-200 rounded-lg p-6 mb-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="bg-white border border-purple-200 rounded-lg p-4 text-center">
                      <i className="fas fa-flag text-purple-600 text-2xl mb-2"></i>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Team Name</label>
                      <p className="text-lg font-bold text-gray-900">{teamInfo.team_name}</p>
                    </div>
                    <div className="bg-white border border-purple-200 rounded-lg p-4 text-center">
                      <i className="fas fa-chalkboard-teacher text-purple-600 text-2xl mb-2"></i>
                      ...
                      <p className="text-sm text-gray-600">{teamInfo.leader_faculty_id}</p>
                    </div>
                    <div className="bg-white border border-purple-200 rounded-lg p-4 text-center">
                  ...
                  {teamInfo.participants && (
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {/* Team Leader Card */}
                        <div className="bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-lg p-4 flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-purple-100 text-sm">{teamInfo.leader_faculty_id}</p>
                            <p className="text-purple-100 text-xs">Team Leader</p>
                          </div>
                        </div>
                        ...
                        {teamInfo.participants.map((participant, index) => (
                          <div key={index} className="bg-white border border-gray-200 rounded-lg p-4 flex items-center">
                            <div className="w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center text-gray-600 font-bold text-lg mr-4">
                              {participant.full_name ? participant.full_name[0].toUpperCase() : participant.faculty_id[0]}
                            </div>
                            <div>
                              <h3 className="font-semibold text-gray-900">{participant.full_name || 'Name not available'}</h3>
                              <p className="text-gray-600 text-sm">{participant.faculty_id}</p>
              ...
              {/* Faculty & Event Details Grid */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Faculty Information */}
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                  <h4 className="font-semibold text-gray-900 mb-3 flex items-center">
                    <i className="fas fa-chalkboard-teacher text-purple-600 mr-2"></i>
                    Faculty Information
                  </h4>
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium font-mono">{registration.faculty_id || user?.faculty_id || user?.employee_id}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{registration.designation || user?.designation || 'N/A'}</span>
                    </div>
                  </div>
                </div>
...
export default FacultyAlreadyRegistered;
