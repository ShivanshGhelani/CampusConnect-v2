...
class StatisticsManager:
    ...
    async def get_platform_statistics() -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                "platform_rating": 4.5  # Default fallback rating
            }
            ...
            for event in events:
                ...
                try:
                    ...
                    if event_collection is not None:
                        # Count attendance records (students who attended)
                        attendance_count = await event_collection.count_documents({
                            "attendance_status": "present"
                        })
                        total_certificates += attendance_count
                except Exception as e:
                    logger.debug(f"Could not access database for event {event_id}: {e}")
                    continue
            ...
            for event in events:
                ...
                try:
                    ...
                    if event_collection is not None:
                        ...
                        feedback_cursor = event_collection.find({})
    ...
    async def get_event_statistics(cls, event_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            registrations = await event_collection.count_documents({"type": "registration"})
            attendance = await event_collection.count_documents({"type": "attendance"})
            feedback = await event_collection.count_documents({"type": "feedback"})
            certificates = await event_collection.count_documents({"type": "certificate"})
