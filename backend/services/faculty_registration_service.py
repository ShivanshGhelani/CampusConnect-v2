...
class FacultyRegistrationService:
    ...
    async def register_individual(
        self, employee_id: str, event_id: str, additional_data: Dict[str, Any] = None
        ...
        try:
            ...
            faculty_data, event_data = await self._get_faculty_and_event_data(
            ...
            registration_id = (
                additional_data.get("registration_id") if additional_data else None
            )
            if not registration_id:
                ...
                logger.warning(
                    f"No registration_id from frontend, generated: {registration_id}"
                )
            else:
                logger.info(
                    f"Using frontend-generated registration_id: {registration_id}"
                )
            ...
            # Update faculty document - add event to event_participations (plural)
            try:
                    ...
                    {"employee_id": employee_id},
                    {
                        "$addToSet": {
                            "event_participations": {
                                ...
                                "registration_id": registration_id,
                                "registration_type": "individual",
                                "registration_date": registration_doc["registration"][
                                    "registered_at"
                                ],
                                "status": "registered",
                            }
            ...
            logger.info(
                f"Faculty individual registration successful: {registration_id}"
            )
    ...
    async def register_team(
        self, team_leader_employee_id: str, event_id: str, team_data: Dict[str, Any]
    ) -> RegistrationResponse:
        """
        ...
        try:
            ...
            logger.info(
                f"Faculty team registration: {team_name} -> {event_id} ({len(team_members)} members)"
            )

            # Validate team members
            if not team_members or team_leader_employee_id not in team_members:
                return RegistrationResponse(
                    success=False, message="Invalid team composition"
                )
            ...
            # Get event data
            _, event_data = await self._get_faculty_and_event_data(
                team_leader_employee_id, event_id
            )
            if not event_data:
                return RegistrationResponse(success=False, message="Event not found")
            ...
            for employee_id in team_members:
                # Get faculty data
                faculty_data, _ = await self._get_faculty_and_event_data(
                    employee_id, event_id
                )
                if not faculty_data:
                    continue
                ...
                if member_reg_id:
                    ...
                    logger.warning(
                        f"No team registration_id from frontend, generated: {registration_id}"
                    )
            ...
            logger.info(
                f"Faculty team registration successful: {team_name} ({len(team_registration_ids)} members)"
            )
            ...
            logger.info(
                f"Updated faculty and event documents for team registration: {team_name}"
            )
            ...
            return RegistrationResponse(
                success=False, message=f"Faculty team registration failed: {str(e)}"
            )

    async def cancel_registration(
        self, employee_id: str, event_id: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            registration = await DatabaseOperations.find_one(
                self.collection,
                {"faculty.employee_id": employee_id, "event.event_id": event_id},
            )
            ...
            registration_type = registration.get("registration", {}).get(
                "type", "individual"
            )

            logger.info(
                f"🔍 FACULTY CANCEL: Found registration {registration_id}, type: {registration_type}"
            )
            ...
            if not delete_result:
                logger.error(
                    f"🔍 FACULTY CANCEL: Delete failed - registration not found"
                )
                return {
                    "success": False,
            ...
            # 2. Remove from faculty's event_participations array (plural)
            try:
                faculty_update_result = await DatabaseOperations.update_one(
                    "faculties",
                    {"employee_id": employee_id},
                    {"$pull": {"event_participations": {"event_id": event_id}}},
                )
                logger.info(
                    f"Removed participation from faculty document: {faculty_update_result}"
                )
            except Exception as e:
                logger.error(
                    f"Failed to update faculty document during cancellation: {e}"
                )

            # 3. Update event document - decrement stats and remove from participated list
            try:
                # Determine which counters to decrement
                decrement_update = {"registration_stats.total_participants": -1}
                ...
                logger.info(
                    f"Updated event document during faculty cancellation: {event_update_result}"
                )
            except Exception as e:
                logger.error(
                    f"Failed to update event document during faculty cancellation: {e}"
                )

            logger.info(
                f"Faculty registration cancellation completed: {registration_id}"
            )
    ...
    async def _get_faculty_and_event_data(
        ...
        faculty_data = await DatabaseOperations.find_one(
            self.faculties_collection, {"employee_id": employee_id}
        )

        event_data = await DatabaseOperations.find_one(
            self.events_collection, {"event_id": event_id}
        )
    ...
    async def _create_registration_document(
        ...
        registration_doc = {
            ...
            "attendance": self._create_attendance_structure(
                attendance_strategy, event_data
            ),
            "feedback": {
                "submitted": False,
                "rating": None,
    ...
    async def _get_event_attendance_strategy(self, event_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            strategy = AttendanceIntelligenceService.detect_attendance_strategy(
                event_data
            )
    ...
    def _create_attendance_structure(
        self, strategy: str, event_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if strategy == "single_mark":
            base_structure.update({"marked": False, "marked_at": None})
            ...
            base_structure.update(
                {"engagement_periods": [], "total_engagement_time": 0, "active_time": 0}
            )
